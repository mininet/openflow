...
sub send_expect_multi_flow {
	...
	my $flow_mod_pkt =
	  create_flow_mod_from_udp( $ofp, $test_pkt, $in_port, $enums{'OFPP_ALL'}, $max_idle, $wildcards );
	...
	# (send to controller)
	$flow_mod_pkt = create_flow_mod_from_udp( $ofp, $test_pkt, $in_port, $enums{'OFPP_CONTROLLER'}, 2, $wildcards );
