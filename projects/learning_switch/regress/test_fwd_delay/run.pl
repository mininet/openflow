...
if ( !( $pid = fork ) ) {
		...
		print "Time required for sending to an unknown destination: @load_time \n";
		...
		@load_time = tv_interval(\@start_time);
		print "Time required for sending to a known destination: @load_time \n";
