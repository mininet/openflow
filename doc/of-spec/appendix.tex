...
When the \verb|OFPFF_CHECK_OVERLAP| flag is set, the switch must check there are no conflicting entries with the same priority. If there is one, the mod fails and an error code is returned. Support for this flag is required in an OpenFlow switch
