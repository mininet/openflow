...
\verb|DESC_STR_LEN| is \input{define/DESC_STR_LEN}and \verb|SERIAL_NUM_LEN| is \input{define/SERIAL_NUM_LEN}.  
...
The \verb|data| field adds detail on why the error occurred.   
\\\\
Bad Request error \verb|code|s include the following:
...
\input{struct/ofp_vendor}
The Vendor message provides a standard way for OpenFlow switches to offer additional functionality within the OpenFlow message type space.  This is a staging area for features meant for future OpenFlow revisions.  If the MSB of the \verb|vendor| field is 0, then then the low-order bytes are an IEEE Organizationally Unique Identifier (OUI), otherwise they are assigned by the OpenFlow Consortium.  The meaning of the data portion of Vendor messages is left undefined.
