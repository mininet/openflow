...
The \verb|max_len| indicates the maximum amount of data from a packet that should be sent when the port is \verb|OFPP_CONTROLLER|.  If \verb|max_len| is zero, the switch must send a zero-size packet_in message.  The \verb|port| specifies the physical port from which packets should be sent. 
 \\\\
