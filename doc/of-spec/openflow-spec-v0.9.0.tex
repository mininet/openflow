...
\begin{figure}[!hbp]
\centering
\includegraphics[height=3in]{header_parsing_flowchart}
\caption{Flowchart showing how header fields are parsed for matching.}
\label{fig:header_parsing}
\end{figure}

The flow table is checked for a matching flow entry.  The header fields used for the table lookup depend on the packet type as described below (and shown in Figure \ref{fig:header_parsing}).
